 ...
 */
function createWebServer({ configStore, orchestrator, historyRecorder, token, logger, onAutostart }) {
  ...
  // 周期推送（每秒），保证前端持续看到实时 CPU/空闲
  const beat = setInterval(() => {
    if (clients.size > 0 && orchestrator && orchestrator.getRuntime) {
      broadcast({ type: 'runtime', runtime: orchestrator.getRuntime() });
    }
  }, 1000);
